...
| JSON-RPC and providers | scheduled | `execution-apis` pinned in `spec-lock.toml`; typed methods, HTTP/WS/IPC/EIP-1193 transports, batching, subscriptions, middleware, quorum/proof trust, and full transaction lifecycle are assigned to `v0.92.0..=v0.108.0`. |
...
| Storage and canonical chain | scheduled | Database contracts, chain/state schemas, crash consistency, migrations, pruning/archive/history expiry, canonical import/reorg, heads, payload invalidation, runtime, and performance are assigned to `v0.130.0..=v0.140.0`. |
| Engine API | scheduled | Complete Engine types and validation are assigned to `v0.143.0`; client/server transport and payload lifecycle are assigned to `v0.144.0`. |
| SSZ beacon consensus and light client | scheduled | SSZ, beacon types, Engine/Beacon API boundaries, weak-subjectivity bootstrap, BLS committees, rotation/persistence, finality scoring, execution proof binding, recovery, complete light-client vectors, and PeerDAS primitives are assigned to `v0.141.0..=v0.153.0`. |
| DevP2P RLPx and discovery | scheduled | Threat/dependency admission, discovery/RLPx, eth/snap messages, peer management, request scheduling, txpool, sync, Portal/history acquisition, and hardening are assigned to `v0.154.0..=v0.164.0`. |
...
| Full beacon state transition | scheduled | Fork registry, complete SSZ/BLS client surfaces, committees/domains, per-slot/per-epoch processing, validator lifecycle, rewards/penalties, operations, execution requests, data availability, fork upgrades, and official vectors are assigned to `v0.189.0..=v0.203.0`. |
| Consensus fork choice and beacon storage | scheduled | Transactional LMD-GHOST/Casper FFG, proposer boost/reorg policy, optimistic invalidation, persistence, operation pools, hot/finalized stores, state reconstruction, custody retention, migration, and repair are assigned to `v0.204.0..=v0.213.0`. |
| Consensus networking and sync | scheduled | Separate consensus discv5/libp2p/GossipSub/ReqResp networking, validation, scoring, backpressure, checkpoint/weak-subjectivity sync, head/range sync, backfill, optimistic recovery, and PeerDAS custody sync are assigned to `v0.214.0..=v0.224.0`. |
| Beacon node Engine DA and APIs | scheduled | Authenticated multi-execution-client coordination, cell/data-column reconstruction, availability tracking, beacon-node orchestration, Beacon REST/Event APIs, and Validator API are assigned to `v0.225.0..=v0.231.0`. |
| Validator client and slashing protection | scheduled | Duty safety, proposals, attestations, aggregation, sync committees, lifecycle operations, doppelganger detection, transactional slashing protection, EIP-3076, validator signers, and Keymanager API are assigned to `v0.232.0..=v0.241.0`. |
| Builder and consensus operations | scheduled | Builder API, relay multiplexing, local fallback, operations/configuration/health surfaces, database tools, deterministic simulation, Hive, multi-execution-client interoperability, and a long-running validator testnet are assigned to `v0.242.0..=v0.249.0`. |
| Complete production assurance | scheduled | Consensus performance, Kani proofs, transition/fork-choice/network/sync/DA/validator/slashing/builder/storage/API audits, remediation, final API freeze, publication rehearsal, and the production candidate are assigned to `v0.250.0..=v0.260.0`. |
