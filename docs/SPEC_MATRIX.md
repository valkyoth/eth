...
| Consensus networking and sync | scheduled | Separate consensus discv5/libp2p/GossipSub/ReqResp networking, validation, scoring, backpressure, checkpoint/weak-subjectivity sync, head/range sync, backfill, optimistic recovery, and PeerDAS custody sync are assigned to `v0.215.0..=v0.225.0`. |
...
| Complete production assurance and admission | scheduled | Quantitative attribution and resource gates, mandatory Hive/client matrices, long testnet, performance, Kani proofs, SSZ/BLS/PeerDAS and integration audits, remediation, public API/crate stability freeze, RC-aware release tooling, exact archive publication, and candidate admission are assigned to `v0.258.0..=v0.274.0` plus `v1.0.0-rc.N`. |
