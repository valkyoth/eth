...
Status: source revisions pinned for `v0.37.3`; scalar, list, and canonical
...
reviewed `k256` adapter behind an explicit feature.
...
| Core dependency independence | audited, first remediation complete | `v0.37.2` audits every dependency that touches core Ethereum behavior, classifying `k256` as default temporary debt, `subtle` as a reviewed-exception candidate, `tiny-keccak`, `serde`, `serde_json`, and `sanitization` as optional paths, and `alloy-rlp` plus `sha3` as dev/reference paths. `v0.37.3` removes default `k256`, removes direct verify-test `sha3`, adds the `RecoverableSecp256k1` boundary, and keeps the reviewed adapter under `secp256k1-k256`; `v0.37.4` and `v0.37.5` cover constant-time/reference and optional parser/sanitization policy. |
