...
Status: source revisions pinned for `v0.15.0`; scalar, list, and canonical
...
EIP-4844 blob transaction decoding implemented.
...
upstream `HEAD` on 2026-07-01 for `v0.15.0`; execution-apis and
...
| EIP-2718 typed transactions | partial | `ethereum/EIPs` pinned in `spec-lock.toml`; envelope classification implemented; EIP-2930 type `0x01`, EIP-1559 type `0x02`, and EIP-4844 type `0x03` field decode implemented; later typed transaction payloads remain opaque |
...
| EIP-4844 blob transactions | field decode | EIP-4844 defines type `0x03`, fourteen payload fields, required 20-byte `to`, max blob fee, and blob versioned hash list; v0.15.0 decodes fields into an unvalidated model without signature, sender, blob fee, KZG, data availability, blob-hash version, blob count, chain, account-state, block blob-gas, or fork validation |
