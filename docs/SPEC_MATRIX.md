...
| SSZ beacon consensus and light client | scheduled | SSZ, beacon types, Beacon API, weak-subjectivity bootstrap, BLS committees, rotation/persistence, finality scoring, execution proof binding, recovery, complete vectors, and PeerDAS are assigned to `v0.141.0..=v0.153.0`. |
...
| Production assurance | scheduled | Platform/performance evidence, Kani, Miri/sanitizers, compatibility gates, task documentation, four audit tracks, remediation, and release-candidate evidence are assigned to `v0.175.0..=v0.188.0`. |
