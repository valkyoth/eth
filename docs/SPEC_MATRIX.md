...
and `v0.40.0` adds the dependency-free native EVM core type foundation,
without admitting a concrete execution backend.
...
| Native EVM execution | core types | `v0.40.0` adds `eth-valkyoth-evm-core` with first-party dependency-free EVM word, stack, borrowed memory, program-counter, opcode, fork, opcode-table skeleton, and deterministic error domains. No opcode execution, gas accounting, state access, calls/create, precompile, or official state-test execution claim is made yet; `v0.41.0` through `v0.47.0` build those layers in audited passes. REVM remains reference/compatibility only if admitted. |
