...
| `v0.40.0..=v0.52.33` and `v0.69.0..=v0.91.0` | Build the first-party EVM, correctness/resource boundaries, advanced precompiles, early core cryptography and proofs, shared decode/session-time/atomically-reserved-evidence contracts, signing/transport separation, non-forgeable constant-size validation contexts, authority-tagged consensus/wire/operational-limit domains, conserved hierarchical evidence capabilities, complete state transition, conformance, tracing, and simulation; REVM remains reference or compatibility only if admitted. |
