...
| `v0.46.0` through `v0.63.0` | Admit or implement cryptographic precompiles only with conformance vectors, dependency review, fuzzing, and pentest gates. Releases through `v0.51.0` complete native SHA-256, RIPEMD-160, bounded ModExp, BN254, and BLAKE2F plus caller-boundary ECRECOVER. `v0.52.0` fixes exact KZG/BLS planning and admission policy; `v0.52.1` defines BLS wire domains; `v0.54.0..=v0.55.0` harden the precompile contract and complete ModExp; and `v0.56.0..=v0.63.0` build first-party BLS12-381 execution. |
