...
| `v0.40.0..=v0.109.0` and `v0.129.0..=v0.173.0` | Build the first-party EVM, correctness/resource boundaries, advanced precompiles, early core cryptography and proofs, shared decode/session-time/atomically-reserved-evidence contracts, signing/transport separation, non-forgeable constant-size validation contexts, authority-tagged consensus/wire/operational-limit domains, conserved hierarchical evidence capabilities, explicit operational collection modes, immutable optional-sink access, allocation-free evidence hot paths, measurable overhead gates, contained public APIs, capability-backed ABA-safe arenas, structurally equivalent benchmarks, lifecycle-complete measurement boundaries, exact runtime-mode dispatch with physical-capacity-class resource charging, fail-closed generation retirement, complete state transition, conformance, tracing, and simulation; REVM remains reference or compatibility only if admitted. |
| `v0.46.0` through `v0.70.0` | Admit or implement cryptographic precompiles only with conformance vectors, dependency review, fuzzing, and pentest gates. Releases through `v0.51.0` establish native SHA-256, RIPEMD-160, initially bounded ModExp, BN254, and BLAKE2F plus caller-boundary ECRECOVER. `v0.52.0` fixes exact KZG/BLS planning and admission policy; `v0.52.1` defines BLS wire domains; `v0.54.0..=v0.55.0` harden the precompile contract and remove the ModExp private cap; and `v0.56.0..=v0.70.0` build first-party BLS12-381 execution. |
| `v0.143.0..=v0.150.0` | Build first-party trusted-setup handling, KZG arithmetic/proof verification, point-evaluation execution, and blob integration before blob consensus validation is claimed. |
| `v0.289.0..=v0.291.0` | Add Kani proof harnesses as extra assurance for selected codec, primitive, evidence-capability conservation, typestate, EVM, trie, state, and cryptographic arithmetic invariants. |
