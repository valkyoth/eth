...
| `v0.40.0..=v0.52.32` and `v0.69.0..=v0.91.0` | Build the first-party EVM, correctness/resource boundaries, advanced precompiles, early core cryptography and proofs, shared decode/session-time/atomically-reserved-evidence contracts, signing/transport separation, non-forgeable constant-size validation contexts, authority-tagged consensus/wire/operational-limit domains, complete state transition, conformance, tracing, and simulation; REVM remains reference or compatibility only if admitted. |
...
| `v0.177.0..=v0.178.1` | Add Kani proof harnesses as extra assurance for selected codec, primitive, typestate, EVM, trie, state, and cryptographic arithmetic invariants. |
