...
| `v0.46.0` through `v0.52.0` | Admit or implement cryptographic precompile backends only with conformance vectors, dependency review, fuzzing where applicable, and pentest gates. `v0.46.0` begins this with first-party dependency-free SHA-256 and RIPEMD-160. `v0.47.0` adds ECRECOVER execution through caller-provided secp256k1 and Keccak traits without adding default crypto dependencies. |
