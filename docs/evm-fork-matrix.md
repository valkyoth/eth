...
G2 curve validation. `v0.50.1` adds G2 subgroup validation, and `v0.50.2` adds
the Fp6/Fp12 tower foundation while non-empty pairing algebra remains
fail-closed. The registry
...
| BN254 add/mul/pairing | `0x06..=0x08` | Byzantium | Add and scalar multiplication execute dependency-free with canonical field and point validation. Pairing validates bounded frames, G2 curve membership, and G2 subgroup membership, and executes empty input; non-empty pairing algebra fails closed until `v0.50.3` and `v0.50.4`. |
