...
`v0.50.0` adds the BN254 pairing frame boundary with empty-input execution,
G2 curve validation, and non-empty fail-closed behavior. The registry
...
| BN254 add/mul/pairing | `0x06..=0x08` | Byzantium | Add and scalar multiplication execute dependency-free with canonical field and point validation. Pairing validates bounded frames and executes empty input; non-empty pairing algebra fails closed until `v0.50.1` through `v0.50.3`. |
