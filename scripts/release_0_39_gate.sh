...
mkdir -p target
cargo tree -p eth -e normal > target/release_0_39_default_runtime_tree.txt
cargo tree -p eth --no-default-features --features evm -e normal > target/release_0_39_evm_tree.txt
cargo tree -p eth -e features --all-features > target/release_0_39_all_features_tree.txt
