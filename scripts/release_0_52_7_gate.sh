...
scripts/checks.sh
scripts/validate-release-readiness.sh v0.52.7
...
mkdir -p target
cargo tree -p eth -e normal > target/release_0_52_7_default_runtime_tree.txt
cargo tree -p eth -e features --all-features > target/release_0_52_7_all_features_tree.txt
scripts/release_crates.py --check
cargo test -p eth-valkyoth-evm --all-features
cargo test --workspace --all-features
cargo clippy --workspace --all-targets --all-features -- -D warnings
cargo clippy --manifest-path fuzz/Cargo.toml --bin execution_admission -- -D warnings
