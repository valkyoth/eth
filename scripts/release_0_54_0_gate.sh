...
scripts/checks.sh
scripts/validate-release-readiness.sh v0.54.0
...
mkdir -p target
cargo tree -p eth -e normal > target/release_0_54_0_default_runtime_tree.txt
cargo tree -p eth -e features --all-features > target/release_0_54_0_all_features_tree.txt
scripts/release_crates.py --check
cargo test -p eth-valkyoth-evm-core --all-features
cargo run -p eth-valkyoth-evm-core --release --features std --example precompile_contract_benchmark
