...
scripts/checks.sh
scripts/validate-release-readiness.sh v0.52.2
...
mkdir -p target
cargo tree -p eth -e normal > target/release_0_52_2_default_runtime_tree.txt
cargo tree -p eth --no-default-features --features evm -e normal > target/release_0_52_2_evm_tree.txt
cargo tree -p eth --no-default-features --features evm-core -e normal > target/release_0_52_2_evm_core_tree.txt
cargo tree -p eth -e features --all-features > target/release_0_52_2_all_features_tree.txt
scripts/release_crates.py --check
cargo test -p eth-valkyoth-evm-core truncated_push --all-features
