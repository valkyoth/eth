...
mkdir -p target
cargo tree -p eth -e normal > target/release_0_52_3_default_runtime_tree.txt
cargo tree -p eth --no-default-features --features evm -e normal > target/release_0_52_3_evm_tree.txt
cargo tree -p eth --no-default-features --features evm-core -e normal > target/release_0_52_3_evm_core_tree.txt
cargo tree -p eth -e features --all-features > target/release_0_52_3_all_features_tree.txt
scripts/release_crates.py --check
cargo test -p eth-valkyoth-codec -p eth-valkyoth-protocol -p eth-valkyoth-verify --all-features
cargo run -p eth-valkyoth-codec --release --example decode_session_benchmark
cargo test --workspace --all-features
cargo clippy --workspace --all-targets --all-features -- -D warnings
cargo clippy --manifest-path fuzz/Cargo.toml --bin decode_limits -- -D warnings
