...
def verify_execution_tests(checkout: Path, lock: dict) -> None:
    expected_repo = lock.get("execution_tests_repo")
    expected_rev = lock.get("execution_tests_rev")
    if not isinstance(expected_repo, str) or not isinstance(expected_rev, str):
...
def run_rlp_fixtures(execution_tests: Path) -> None:
    ...
    env = {**os.environ, "ETH_EXECUTION_TESTS_RLP_DIR": str(rlp_dir)}
    subprocess.run(
        [
            "cargo",
            "test",
