...
mkdir -p target
cargo tree -p eth -e normal > target/release_0_44_default_runtime_tree.txt
cargo tree -p eth --no-default-features --features evm -e normal > target/release_0_44_evm_tree.txt
cargo tree -p eth --no-default-features --features evm-core -e normal > target/release_0_44_evm_core_tree.txt
cargo tree -p eth -e features --all-features > target/release_0_44_all_features_tree.txt
