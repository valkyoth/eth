...
mkdir -p target
cargo tree -p eth -e normal > target/release_0_38_default_runtime_tree.txt
cargo tree -p eth --no-default-features --features evm -e normal > target/release_0_38_evm_tree.txt
cargo tree -p eth -e features --all-features > target/release_0_38_all_features_tree.txt
