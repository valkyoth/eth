...
plus explicit state reads. Identity, SHA-256, RIPEMD-160, bounded ModExp, and
BN254 add/mul execute without dependencies; ECRECOVER executes through
explicit caller-provided secp256k1 and Keccak boundaries. Remaining
cryptographic precompiles are admitted as fail-closed descriptors until
reviewed backends or first-party implementations are added.
...
- No block parser yet.
- Identity, SHA-256, RIPEMD-160, ECRECOVER, bounded ModExp, and BN254 add/mul
  precompile execution are implemented; remaining cryptographic precompile
  work is scheduled for `v0.50.0` through `v0.52.0`.
...
```toml
[dependencies]
eth = { version = "0.49.0", features = ["sanitization"] }
```
...
```toml
[dependencies]
eth = { version = "0.49.0", features = ["keccak-tiny"] }
```
...
```toml
[dependencies]
eth = { version = "0.49.0", features = ["secp256k1-k256"] }
```
...
```toml
[dependencies]
eth = { version = "0.49.0", features = ["evm"] }
```
...
bounded ModExp, BN254 add/mul, and ECRECOVER can execute now; ECRECOVER
requires caller-provided secp256k1 and Keccak backends. ModExp uses a
first-party no-alloc engine with an explicit release operand cap. BN254 add/mul
uses first-party fixed-size field arithmetic with canonical field and point
validation. Remaining cryptographic precompiles are exposed as bounded plans
and return a backend-unavailable error until audited backends or first-party
...
| `eth-valkyoth-evm-core` | no | Dependency-free native EVM core domains plus gas-metered basic bounded opcode execution, explicit host-state reads, fail-closed call/create planning, and bounded identity/SHA-256/RIPEMD-160/ECRECOVER/ModExp/BN254 add/mul precompile execution. |
...
Compatibility evidence for `0.49.0`:
