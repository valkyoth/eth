...
plus explicit state reads. Identity, SHA-256, RIPEMD-160, and bounded ModExp
execute without dependencies; ECRECOVER executes through explicit
caller-provided secp256k1 and Keccak boundaries. Remaining cryptographic
precompiles are admitted as fail-closed descriptors until reviewed backends or
first-party implementations are added.
...
- No block parser yet.
- Identity, SHA-256, RIPEMD-160, ECRECOVER, and bounded ModExp precompile
  execution are implemented; remaining cryptographic precompile work is
  scheduled for `v0.49.0` through `v0.52.0`.
...
```toml
[dependencies]
eth = { version = "0.48.0", features = ["sanitization"] }
```
...
```toml
[dependencies]
eth = { version = "0.48.0", features = ["keccak-tiny"] }
```
...
```toml
[dependencies]
eth = { version = "0.48.0", features = ["secp256k1-k256"] }
```
...
```toml
[dependencies]
eth = { version = "0.48.0", features = ["evm"] }
```
...
bounded ModExp, and ECRECOVER can execute now; ECRECOVER requires
caller-provided secp256k1 and Keccak backends. ModExp uses a first-party
no-alloc engine with an explicit release operand cap. Remaining cryptographic
precompiles are exposed as bounded plans and return a backend-unavailable error
...
| `eth-valkyoth-evm-core` | no | Dependency-free native EVM core domains plus gas-metered basic bounded opcode execution, explicit host-state reads, fail-closed call/create planning, and bounded identity/SHA-256/RIPEMD-160/ECRECOVER/ModExp precompile execution. |
...
Compatibility evidence for `0.48.0`:
