...
- Call/create opcodes are recognized, stack/memory/policy validated, and then
  ...
  state commits occur.
- Precompile descriptors are fork-aware. Identity can execute without
  dependencies; cryptographic precompiles are bounded plans only and fail
  closed until audited backends are admitted.
- Unsupported opcodes and unsupported forks are rejected with named errors.
- No nested call/create execution, log, cryptographic precompile, refund, or committed
  storage-write semantics are claimed yet.
